                ...
                @foreach ($peminjamans as $userId => $loans)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-3">
                            ...
                            @endphp
                            <span
                                class="bg-{{ $status === 'Menunggu Persetujuan' ? 'yellow' : ($status === 'Dipinjamkan' ? 'green' : 'red') }}-100 text-{{ $status === 'Menunggu Persetujuan' ? 'yellow' : ($status === 'Dipinjamkan' ? 'green' : 'red') }}-800 text-xs px-2.5 py-0.5 rounded">
                                {{ $status }}
                            </span>
                        </td>
