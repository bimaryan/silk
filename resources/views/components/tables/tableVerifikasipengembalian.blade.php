    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400" id="data-peminjaman">
        ...
        <tbody>
            @foreach ($pengembalian as $item)
                @if ($item->peminjaman->status_pengembalian != 'Dipinjam')
                    <tr
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td scope="col" class="px-6 py-3">{{ $loop->iteration }}</td>
                        <td scope="col" class="px-6 py-3">
                            @if ($item->peminjaman->mahasiswa)
                                {{ $item->peminjaman->mahasiswa->nama }}
                            @elseif ($item->peminjaman->dosen)
                                {{ $item->peminjaman->dosen->nama }}
                            @else
                                Tidak ada nama peminjam
                            @endif
                        </td>
                        <td scope="col" class="px-6 py-3">
                            {{ $item->peminjaman->tanggal_waktu_peminjaman }}
                        </td>
                        <td scope="col" class="px-6 py-3">
                            {{ $item->peminjaman->waktu_pengembalian }}
                        </td>
                        <td scope="col" class="px-6 py-3">
                        ...
                        <td scope="col" class="px-6 py-3">
                            <div class="flex items-center space-x-6">
                                <form action="{{ route('peminjaman.update', $item->peminjaman->id) }}" method="POST"
                                    class="flex items-center gap-1">
                                    @csrf
                                    @method('PUT')
                                    <div>
                                        <select name="aprovals" id="aprovals" class="p-1 border rounded-md">
                                            <option value="Belum" {{ $item->peminjaman->aprovals == 'Belum' ? 'selected' : '' }}>
                                                Belum
                                            </option>
                                            <option value="Ya" {{ $item->peminjaman->aprovals == 'Ya' ? 'selected' : '' }}>
                                                Ya
                                            </option>
                                            <option value="Tidak" {{ $item->peminjaman->aprovals == 'Tidak' ? 'selected' : '' }}>
                                                Tidak
                                            </option>
                                        </select>
                                    </div>
